...
{
    public RolePermissionRepository(ApplicationDbContext dbContext, ILogger<RolePermissionRepository> logger) : base(dbContext, logger)
    {
    }

    public async Task<IEnumerable<RolePermission>> GetByRoleIdAsync(long userRoleRoleId)
    {
        List<RolePermission> rolePermissions = await this.FindByCondition(rolePermission => rolePermission.RoleId == userRoleRoleId)
            .ToListAsync();
